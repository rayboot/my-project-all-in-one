...
public class SearchMapActivity extends Activity {
		...
		mSearch.init(app.mBMapManager, new MKSearchListener() {
			...
			public void onGetPoiDetailSearchResult(int type, int error) {
				if (error != 0) {
					...
					Toast.makeText(SearchMapActivity.this, "成功，查看详情页面",
							Toast.LENGTH_SHORT).show();
				}
			}
			...
			public void onGetPoiResult(MKPoiResult res, int type, int error) {
				...
				if (res.getCurrentNumPois() > 0) {
					// 将poi结果显示到地图上
					poiOverlay = new MyPoiOverlay(SearchMapActivity.this,
							mMapView, mSearch);
	...
	public void goToNextPage(View v) {
		...
		if (flag != 0) {
			Toast.makeText(SearchMapActivity.this, "先搜索开始，然后再搜索下一组数据",
					Toast.LENGTH_SHORT).show();
		}
	}
	...
	public void saveToDB(View v) {
		...
		Toast.makeText(this, "已批量保存", Toast.LENGTH_LONG).show();
	}

	public void saveCenterPoint(View v) {
		Intent intent = new Intent(this, CenterSaveActivity.class);
